...
function neteaseTracker(m, a, n, l) {
    ...
    if (d == 1) {
        ...
        function c(r, i) {
            var t = ntes_get_cookie(r), s = ntes_get_cookie(i);
            return t == -1 ? s == -1 ? "" : s : t
        }
...
function neteaseClickTracker(p, m, b, o, k) {
    ...
    var e = a.length, g = 0, f, q, j;
    if (!e) {
        return
    }
    var d = function (i) {
        var r = _ntes_fixEvent(i || window.event), n = this;
    ...
    var l = document.getElementsByTagName("head")[0], c;
    if (l) {
...
function ntes_page_click_stat(k) {
    var m = this.tagName.toLowerCase(), o = _ntes_fixEvent(k || window.event), i = "", q = "", g;
    switch (m) {
        case"a":
        case"area":
    ...
    var r = escape(i), f = escape(q), d = escape(document.URL),
        ...
        l = _ntes_page_data.getRunTime(), s = Math.round(o.pageX), p = Math.round(o.pageY);
...
function ntes_page_unload_stat(i) {
    var d = escape(document.URL),
        ...
        a = document.documentElement.scrollTop || document.body.scrollTop || 0,
        b = document.compatMode === "CSS1Compat" ? document.documentElement.clientHeight : document.body.clientHeight,
        j = _ntes_page_data.getRunTime(), c = Math.max(_ntes_page_data.maxScroll, a), k = _ntes_page_data.maxClick,
...
function neteaseClickStat(c) {
    ...
    var o = Math.random(), g = Math.abs(c || 30);
    o = parseInt(o * g);
    if (o != 0) {
        return
    }
    var h = document.getElementsByTagName("a"), f = h.length, j = 0;
    for (; j < f; j++) {
    ...
    for (; j < f; j++) {
        if (m[j].href.indexOf("javascript:") >= 0) {
            _ntes_bindEvent(m[j], "click", function (n) {
                ...
                var i = (this.href).split("javascript:")[1] || "", a;
                if (i) {
                    a = new Function(i);
                    return a.call(this)
                }
...
function _ntes_fixEvent(f) {
    ...
    if (c.pageX == null && c.clientX != null) {
        var d = document.documentElement, a = document.body;
...
function neteaseClickStatForArea(l, e, c) {
    ...
    var d = Math.random(), g = Math.abs(c || 30);
    d = parseInt(d * g);
    if (d != 0) {
        return
    }
    var m = e || document.URL, b = function (i) {
        ntes_area_click_stat.call(this, i, l, m)
    };
    var h = l.getElementsByTagName("a"), f = h.length, j = 0;
    for (; j < f; j++) {
...
function ntes_area_click_stat(v, k, b) {
    var f = this.tagName.toLowerCase(), q = _ntes_fixEvent(v || window.event), u = "", o = "", x;
    switch (f) {
        case"a":
        case"area":
    ...
    var j = escape(u), c = escape(o), i = escape(b), d = s.width, m = s.height, w = k.scrollLeft || 0,
        r = k.scrollTop || 0, g = _ntes_page_data.getRunTime(), p = Math.round(q.pageX) - a.x + w,
...
var ntes_area_click_tools = {
    getOffset: function (d) {
        ...
        var f = d.getBoundingClientRect(), j = d.ownerDocument, g = j.body, b = j.documentElement,
            e = b.clientTop || g.clientTop || 0, h = b.clientLeft || g.clientLeft || 0,
